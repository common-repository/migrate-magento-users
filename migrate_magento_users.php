...
<ul>
<li>Set Host, Username and Password of Magento database same as wordpress database. </li>
<li>Delete users of wordPress site excluding admin user.</li>
<li>To remove duplication of users ID in users table, please make unique ID of admin user ID in user table and user meta table which are not exists in Magento users IDs.</li>
</ul>
</p>
</div>
<?php
...
if(isset($_POST['submit'])){
	$MagentoDBname_DBPrefix = sanitize_text_field($_POST['MagentoDBname_DBPrefix']);
	$dbExplode 	= explode(".",$MagentoDBname_DBPrefix);
	$dbName 	= $dbExplode[0];
	...
	if($MagentoDBname_DBPrefix != 'MagentoDBname.DBPrefix' ){
		// Check db
		...
		$jUser = $lwpdb->get_results( $lwpdb->prepare( "SELECT
				...
				u.password password
				FROM ".$MagentoDBname_DBPrefix."_user u
				ORDER BY u.user_id","",""));					
		global $wpdb;
		$wpdb->show_errors();
...
$magentoDbName='magentoDBname';
}
?>
<form method="post">
<table>
<tr><th>Insert Magento database name with prefix</th></tr>
<tr><th><span style="color:red;"> (ex - MagentoDBname.DBPrefix) *</span></th><td><input type="text" name="MagentoDBname_DBPrefix" id="MagentoDBname_DBPrefix" onfocus="this.value=='MagentoDBname.DBPrefix'?this.value='':this.value=this.value;" onblur="this.value==''?this.value='MagentoDBname.DBPrefix':this.value=this.value;" value="<?php if(isset($MagentoDBname_DBPrefix)) { echo $MagentoDBname_DBPrefix; } ?>" maxlength="100" size="30px;"></td></tr>
